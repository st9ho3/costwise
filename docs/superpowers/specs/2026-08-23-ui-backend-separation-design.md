...
| 3 | **API is the only door to data.** Web server components and mobile screens call the API over HTTP; nothing outside `apps/api` imports services or repositories. *Amended by Task 2 spec (2026-08-23): the domain layer lives in `packages/domain`, consumed by both apps during the Tasks 2–3 transition; after Task 4, `apps/api` is its only consumer, preserving this ADR's intent.* | Makes mobile and agents first-class clients; every permission check lives in one place | Letting web keep direct service imports (splits the trust boundary in two) |
