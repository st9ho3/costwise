...
> `pnpm fallow` now maps 1:1 to rows 9, 21–31, 33, 34. See the `Outcome` column per row and the
> "Checkpoint amendment" section for details.
...
Task 5 ran to completion, including the checkpoint amendment for rows 34–38 above. See the
`Outcome` column on every row (1–38) for the per-row disposition.
