...
| 16 | **Package-level orphan (1 file):** `packages/domain/src/types/auth.ts` | dead-code | `safe-delete` | `types/auth.ts` is the NextAuth session-type shim, dead since auth moved to Better Auth in `apps/api` — `grep -rn "types/auth"` across `apps/` and `packages/` returns zero hits. |
